...
public class AdminGoodsService {
     ...
     */
    @Transactional
    public Object update(GoodsAllinone goodsAllinone) throws IOException, SolrServerException {
        ...
        // 商品基本信息表litemall_goods在Solr中的数据更新
        if (goodsService.updateByIdInSolr(goods) != 0) {
            throw new RuntimeException("更新Solr数据失败");
        }
    ...
    @Transactional
    public Object delete(LitemallGoods goods) throws IOException, SolrServerException {
        Integer id = goods.getId();
        if (id == null) {
            return ResponseUtil.badArgument();
        }
        ...
        //从Solr中按id删除商品，add by zhangran in 20191022
        goodsService.deleteByIdInSolr(gid);

        return ResponseUtil.ok();
    }

    @Transactional
    public Object create(GoodsAllinone goodsAllinone) throws IOException, SolrServerException {
        Object error = validate(goodsAllinone);
        if (error != null) {
            return error;
        }
        ...
        // 商品基本信息添加至solr
        goodsService.addIntoSolr(goods);
